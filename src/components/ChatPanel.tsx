...
export function ChatPanel({ getFlow, onFlowUpdated, onRunningChange, hasFlow, hasToken, onRequestImport, onRequestToken }: ChatPanelProps) {
  ...
  function handleLauncherClick() {
    if (blocked) { setGateOpen(o => !o); return } // não abre: mostra o que falta
    setOpen(true)
  }
  ...
  if (!open) {
    return (
      <div ref={gateRef} className="fixed bottom-4 right-4 z-30">
        ...
        <button
          ...
          className={`flex items-center gap-2 rounded-full px-4 py-3 text-sm font-semibold shadow-lg transition-colors ${isDark ? 'bg-indigo-600 text-white hover:bg-indigo-500' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}
        >
