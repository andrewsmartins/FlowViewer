...
- O agente opera o **servidor MCP local** (`mcp/server.ts`, stdio) com 10 tools: `create_node`, `set_message`, `set_menu`, `set_action_field`, `set_choices`, `connect`, `connect_to_bot`, `validate`, `revert` e as de leitura/resolução de nomes.
...
mcp/
├── server.ts                   Servidor MCP stdio (`npm run mcp`): expõe 10 tools de leitura/mutação do fluxo
...
│   ├── flowTools.ts            10 tools MCP (create_node, set_message, set_menu, connect, validate…)
